	...
	switch (e.target.name) {
		...
		case "mensaje":
			if (expresiones.mensaje.test(e.target.value)) {
				...
				document
					.querySelector(`#formulario-mensaje .formulario-input-error`)
...
const validarCampo = (expresion, input, campo) => {
	if (expresion.test(input.value)) {
		document
			.getElementById(`formulario-${campo}`)
			.classList.remove("formulario-incorrecto");
		document
			.getElementById(`formulario-${campo}`)
			.classList.add("formulario-correcto");
		document
			.querySelector(`#formulario-${campo} .formulario-input-error`)
			.classList.remove("formulario-input-error-activo");
		campos[campo] = true;
	} else {
		document
...
formulario.addEventListener("submit", (e) => {
	if (campos.nombre && campos.email && campos.asunto && campos.mensaje) {
		...
		e.preventDefault();
		document
			.getElementById("formulario-mensaje-enviado")
			.classList.add("formulario-mensaje-enviado-activado");
		setTimeout(() => {
			document
				.getElementById("formulario-mensaje-enviado")
				.classList.remove("formulario-mensaje-enviado-activado");
		}, 5000);
